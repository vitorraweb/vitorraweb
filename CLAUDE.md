...
- **HttpOnly-cookie auth option (XSS hardening):** Sanctum SPA mode built **alongside** Bearer tokens (`statefulApi()` + `lib/http.ts`). **Off by default**; switch on with backend `SANCTUM_STATEFUL_DOMAINS` + `SESSION_DOMAIN=.vitorra.org` + `SESSION_SECURE_COOKIE=true` and Vercel `NEXT_PUBLIC_AUTH_MODE=cookie`. Reversible.
