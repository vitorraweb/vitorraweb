 ...
 * "Website traffic" card. Cached 5 minutes so the dashboard's 45s poll doesn't
...
class PlausibleService
{
    ...
    public function getStats(): array
    {
        ...
        return Cache::remember('plausible_stats', 300, function () use ($apiKey, $siteId) {
            try {
                ...
                return [
                    ...
                    'top_pages'       => collect($pages['results'] ?? [])->map(fn ($row) => [
                        'page'     => $row['dimensions'][0] ?? '/',
                        'visitors' => (int) ($row['metrics'][0] ?? 0),
                    ])->values()->all(),
                    'top_sources' => collect($sources['results'] ?? [])->map(fn ($row) => [
                        'source'   => $row['dimensions'][0] ?: 'Direct / None',
                        'visitors' => (int) ($row['metrics'][0] ?? 0),
                    ])->values()->all(),
                ];
            } catch (\Throwable $e) {
                Log::warning('Plausible stats fetch failed: ' . $e->getMessage());
    ...
    private function query(string $apiKey, string $siteId, array $body): array
    {
        ...
        if (! $res->successful()) {
            throw new \RuntimeException("Plausible API error ({$res->status()}): {$res->body()}");
        }
