...
use App\Http\Controllers\Api\FetTrialController;
use App\Http\Controllers\Api\FetTrialImportController;
use App\Http\Controllers\Api\FetTrialShareController;
...
Route::middleware('auth:sanctum')->group(function () {
    ...
    */
    Route::middleware(['role:admin,ops', 'ability:admin'])->prefix('admin')->group(function () {
        ...
        /*
         * Client FET trials — the pre-sale proof loop. Separate module from
         * `fet` above: marketing runs the trials and has this by default, while
         * `fet` is operations' post-sale savings record.
         */
        Route::middleware('perm:fet_trials')->group(function () {
            Route::get('/fet-trials', [FetTrialController::class, 'index']);
            Route::post('/fet-trials', [FetTrialController::class, 'store']);
            Route::get('/fet-trials/{trial}', [FetTrialController::class, 'show']);
            Route::match(['put', 'patch'], '/fet-trials/{trial}', [FetTrialController::class, 'update']);
            Route::delete('/fet-trials/{trial}', [FetTrialController::class, 'destroy']);
            Route::post('/fet-trials/{trial}/revalidate', [FetTrialController::class, 'revalidate']);

            // Trips
            Route::post('/fet-trials/{trial}/trips', [FetTrialController::class, 'storeTrip']);
            Route::match(['put', 'patch'], '/fet-trials/{trial}/trips/{trip}', [FetTrialController::class, 'updateTrip']);
            Route::delete('/fet-trials/{trial}/trips/{trip}', [FetTrialController::class, 'destroyTrip']);
            Route::post('/fet-trials/{trial}/trips/{trip}/status', [FetTrialController::class, 'setTripStatus']);

            // Data-quality findings
            Route::post('/fet-trials/{trial}/flags/{flag}/resolve', [FetTrialController::class, 'resolveFlag']);
            Route::post('/fet-trials/{trial}/flags/{flag}/reopen', [FetTrialController::class, 'reopenFlag']);

            // Import the client's own export: preview, adjust, then commit.
            Route::get('/fet-trials/{trial}/imports', [FetTrialImportController::class, 'index']);
            Route::post('/fet-trials/{trial}/imports/preview', [FetTrialImportController::class, 'preview']);
            Route::post('/fet-trials/{trial}/imports/repreview', [FetTrialImportController::class, 'repreview']);
            Route::post('/fet-trials/{trial}/imports/commit', [FetTrialImportController::class, 'commit']);

            // Closing the deal — a win creates the post-sale installation and
            // carries the trial's measured baseline into it.
            Route::post('/fet-trials/{trial}/outcome', [FetTrialController::class, 'outcome']);
            Route::post('/fet-trials/{trial}/reopen', [FetTrialController::class, 'reopen']);
