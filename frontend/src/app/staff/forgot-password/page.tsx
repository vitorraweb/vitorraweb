...
export default function StaffForgotPasswordPage() {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      await apiStaff("/auth/forgot-password", { method: "POST", body: JSON.stringify({ email }) });
      setSent(true);
    } catch (err: unknown) {
        ...
        <div className="bg-white rounded-[24px] p-7 shadow-2xl" style={{ border: "1px solid rgba(197,178,122,0.18)" }}>
          {sent ? (
            <div className="text-center py-2">
              ...
              <Link href="/staff/login" className="inline-block mt-6 text-sm font-semibold" style={{ color: "#7A6020" }}>
                Back to sign in
              </Link>
            </div>
          ) : (
            <form onSubmit={submit} className="space-y-5">
              ...
              <p className="text-center text-sm">
                <Link href="/staff/login" style={{ color: "#7A6020" }} className="font-semibold">Back to sign in</Link>
              </p>
            </form>
          )}
        </div>
