  ...
  if (list.length === 0) {
    return (
      <div className="bg-white rounded-[28px] border border-black/[0.05] shadow-card p-14 text-center">
        ...
        <p className="text-base font-semibold mb-1" style={{ color: "#1E1E1E" }}>{t("noOrders")}</p>
        <p className="text-sm" style={{ color: "#999" }}>{t("noOrdersSub")}</p>
      </div>
    );
  }
          ...
          <Link key={o.id} href={`/account/orders/${o.reference}`} className="group bg-white rounded-[20px] border border-black/[0.05] shadow-card p-5 flex items-center gap-4 hover-lift">
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2.5 mb-1 flex-wrap">
                ...
                <span className="text-[10px] font-bold uppercase tracking-wide px-2.5 py-1 rounded-full" style={{ background: sc.bg, color: sc.fg }}>{STATUS_KEY[o.status] ? t(STATUS_KEY[o.status]) : o.status}</span>
              </div>
              <p className="text-xs" style={{ color: "#999" }}>{date(o.created_at)} · {t("paymentLabel")} {o.payment_status}</p>
            </div>
