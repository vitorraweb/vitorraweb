  ...
  if (list.length === 0) {
    return (
      <div className="bg-white rounded-[28px] border border-black/[0.05] shadow-card p-14 text-center">
        ...
        <p className="text-base font-semibold mb-1" style={{ color: "#1E1E1E" }}>{t("noEnquiries")}</p>
        <p className="text-sm mb-6" style={{ color: "#999" }}>{t("noEnquiriesSub")}</p>
        <Link href="/enquire" className="btn-primary">{t("makeEnquiry")}<ArrowRight className="w-4 h-4" /></Link>
      </div>
    );
  }
        ...
        return (
          <div key={e.id} className="bg-white rounded-[20px] border border-black/[0.05] shadow-card p-6">
            <div className="flex items-center gap-2.5 mb-2.5 flex-wrap">
              <span className="text-xs font-semibold" style={{ color: "#1E1E1E" }}>{e.product_category ? (LABEL_KEY[e.product_category] ? t(LABEL_KEY[e.product_category]) : e.product_category) : t("generalEnquiry")}</span>
              <span className="text-[10px] font-bold uppercase tracking-wide px-2.5 py-1 rounded-full" style={{ background: sc.bg, color: sc.fg }}>{STATUS_KEY[e.status] ? t(STATUS_KEY[e.status]) : e.status.replace(/_/g, " ")}</span>
